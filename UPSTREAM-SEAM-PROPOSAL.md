...
`dsh-liveai-talk` proves the shape works: it defines these three seams and
...
`dsh-liveai-talk` does with the `liveai-video` job kind.

## Design constraints learned from LiveAI Talk

1. **Credentials belong to `ctx.credentials`, not provider config.** LiveAI Talk
...
3. **Provider selection must not depend on registration order.** LiveAI Talk uses
...
4. **Vendor quirks stay inside providers.** Doubao frame codecs, Volcengine
   ...
   internals in LiveAI Talk; the core only sees normalized events.

## Migration path

1. Land the three Service Definition packages in dsh.
2. Move the codecs and providers from `dsh-liveai-talk` into official
   provider packages, leaving LiveAI Talk as the Consumer/UI.
...
- Implementation reference:
  <https://github.com/eddiehuang227-source/dsh-liveai-talk>

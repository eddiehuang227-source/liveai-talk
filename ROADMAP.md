...
当前状态：**全部目标完成** —— LiveAI Talk 已发布到 GitHub 与 npm，真实 API Key 录制已产出，上游 seam 提案已提交；插件三层结构与全部 provider 就绪，73 个单元测试与真实 dsh 安装冒烟测试通过。

## M1 已完成（本轮）

- [x] `dsh-liveai-talk` 独立子文件夹（AItalk 仓库内）
...
- [x] client：`conversation.view` slot 注册「LiveAI Talk」标签页，显示人物卡
- [x] 测试：单元 73/73；集成测试真实安装 tarball → dump-config → 启动 web → 探针通过
- [x] 浏览器人工验证：dsh Web UI 出现「LiveAI Talk」标签并渲染两位人物

## M2 核心对话管线（已完成）

- [x] 从 `coordinator.py` 抽 `DialoguePipeline` 纯状态机 + `emotion.js` 语义解析（句子切分、情绪/动作、TTS 清洗）
- [x] host 暴露 `/liveai/pipeline`、`/liveai/analyze`、`/liveai/talk`、`/liveai/turn/<sessionId>`
...
- [x] 零 Key TTS：client 端 `liveaiTts` runtime + `browser-tts` SpeechSynthesis provider + 人物卡「试听」
- [x] 零 Key ASR：client 端 `liveaiAsr` runtime + `browser-speech` SpeechRecognition provider + 视图「语音输入」（识别文本回填 `/liveai/talk`）

## M4 视频与实时数字人 seam（已完成）

- [x] `video-jimeng` provider：火山 HMAC 签名 + 即梦 submit/query 迁移，通用 `/liveai/video/submit|status` 路由
- [x] `realtime-volc` provider：`avatar-token` 的 WSS 短期令牌迁移，`/liveai/realtime/volc-token`
- [x] `realtime-vidu` provider：`vidu-live` 的 live session 代理迁移，`/liveai/realtime/vidu/session`
- [x] 长任务接入 `ctx.jobs`：`liveai-video` job kind + `VideoJobRunner`（cancel/done/readOutput），submit 路由返回 jobId；实时与视频 provider 密钥已接入 `ctx.credentials`
...
- [x] GitHub：`https://github.com/eddiehuang227-source/dsh-liveai-talk`（public，topics 含 `dsh-plugin`，tag `v0.2.0`，Release 附 `dsh-liveai-talk-0.2.0.tgz`）
- [x] npm：`dsh-liveai-talk@0.2.0` 已发布到 registry.npmjs.org（tarball：`https://registry.npmjs.org/dsh-liveai-talk/-/dsh-liveai-talk-0.2.0.tgz`）

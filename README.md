...
FlowAct 对话角色可视化核心，以 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)（dsh）bundle 插件的形式发布。
...
当前是**能安装、能显示人物、语音/视频/实时 provider 齐备的切片**：人物卡来自 `/flowact/characters`，UI 通过 dsh slot 注册；host 已注册 `asr-doubao` / `tts-doubao` / `jimeng` / `realtime-volc` / `realtime-vidu` provider（密钥走 `ctx.credentials` 或本机代理），client 提供零 Key `browser-tts` / `browser-speech`。剩余事项是 `ctx.jobs` 长任务接入与发布整理（见 [ROADMAP.md](ROADMAP.md)）。

![dsh Web UI 中的「AI 数字人」视图](docs/screenshot.png)

## 安装（当前 dsh 0.1.0-rc.5 验证）

```sh
dsh plugin --profile avatar add /path/to/dsh-flowact-avatar
dsh --profile avatar --dump-config   # 应看到 flowact-avatar 行
dsh --profile avatar web
```

打开 Web UI，在会话顶部视图标签选择「AI 数字人」。
...
`test:integration` 会创建临时 `DSH_HOME`，安装 tarball，校验 `--dump-config`、启动 web 并请求 `/flowact/health`、`/flowact/characters` 和客户端 bundle。

## 发布

- 仓库添加 topic：`dsh-plugin`；
- npm：`npm publish`（`lib/` 已预构建）；
- tarball：`npm pack`，用户 `dsh plugin add ./dsh-flowact-avatar-0.1.0.tgz`；
